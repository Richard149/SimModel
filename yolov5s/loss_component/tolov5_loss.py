...
def build_targets(p, targets):
    ...
    ## 3 layers
    for i in range(3):
        anchors = g_anchor_tensor[i]
        anchors = anchors.to(targets.device)
        gain[2:6] = torch.tensor(p[i].shape)[[3, 2, 3, 2]]  # xyxy gain，取特征图的wh
        ...
        if nt:
            ...
            ############# target 宽高比与anchor的宽高比的比值小于设定的阈值，则认为是匹配的。
            j = torch.max(r, 1. / r).max(2)[0] < 2.0  # compare
...
def compute_loss(p, targets):  # predictions, targets
    ...
    ### pos_weight 正样本的权重默认1.0；大于1.0会让模型更加关注预测正样本
    ...
    BCEcls, BCEobj = FocalLoss(BCEcls), FocalLoss(BCEobj)
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if n:
            ...
            # Objectness
            tobj[b, a, gj, gi] = (1.0 - 0.2) + 0.2 * iou.detach().clamp(0).type(tobj.dtype)  # iou ratio
